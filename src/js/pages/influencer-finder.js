...
function initializeFilters() {
  ...
  ratingInputs.forEach((input) => {
    input.addEventListener('change', function () {
      logger.info('Rating filter:', this.value);
    });
  });
...
function applySort(sortType) {
  logger.info('Sorting by:', sortType);
...
function updateFilterCount() {
  ...
  logger.info('Active filters:', activeFilters);
}
...
window.applyFilters = function () {
  ...
  const filters = {
    ...
    platforms: Array.from(document.querySelectorAll('.platform-filter input:checked')).map(
      (cb) => cb.id
    ),
    categories: Array.from(document.querySelectorAll('.category-filter input:checked')).map(
      (cb) => cb.id
    ),
