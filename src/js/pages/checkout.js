...
function applyDiscount(percentage) {
  const subtotal = 1648.0;
  const discount = subtotal * (percentage / 100);
  const _newTotal = subtotal - discount + 15.0 + 149.67; // Including shipping and tax

  // Update UI (in real app, would recalculate properly)
  logger.info(`Applied ${percentage}% discount: -$${discount.toFixed(2)}`);
}
...
function initializeCheckoutSteps() {
  ...
  const currentStep = 1; // Starting at shipping
...
function goToStep(stepIndex) {
  logger.info('Going to step:', stepIndex);
