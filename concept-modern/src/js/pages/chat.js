...
document.addEventListener('DOMContentLoaded', function() {
    ...
    chatContacts.forEach((contact, index) => {
        ...
        if (contactId === '1') {
            conversations[contactId].messages = [
                ...
                { text: "Hi Sarah! It's going great. I just finished the dashboard redesign.", type: 'right', time: '2:32 PM' },
                { text: "Here's the latest version for your review", type: 'right', time: '2:33 PM', attachment: { name: 'Dashboard_Design_v2.pdf', icon: 'fa-file-pdf' } },
            ...
            conversations[contactId].messages = [
                ...
                { text: "You're welcome! Let me know if you need anything else.", type: 'right', time: 'Yesterday' }
            ];
        }
    });
    ...
    function createMessageElement(text, type = 'left', timeStr = null, attachment = null) {
        ...
        const time = timeStr || new Date().toLocaleTimeString('en-US', { 
            hour: 'numeric', 
            minute: '2-digit' 
        });
        ...
        if (type === 'right') {
            ...
            const name = activeContact ? activeContact.querySelector('h6').textContent : 'User';
            const avatarElement = activeContact ? activeContact.querySelector('.avatar-circle').cloneNode(true) : null;
    ...
    function updateChatHeader(contactElement) {
        ...
        if (headerStatus) {
            ...
            headerStatus.className = status === 'Online' ? 'text-success' : (status === 'Away' ? 'text-warning' : 'text-muted');
        }
    }
